...
> 07:00 KST 스케줄 자동 발행과 별개로, 운영자가 직접 승인·실행하는 수동 발행 절차를 고정한다.
...
- **스케줄 자동 발행**: 매일 07:00 KST 스케줄이 collect → dedup → synth → validate → draft 커밋까지
  수행한다.
...
KST 저녁에 수동 발행하면 다음 07:00 KST 스케줄과 같은 UTC 날짜가 겹칠 수 있다(주의사항).
